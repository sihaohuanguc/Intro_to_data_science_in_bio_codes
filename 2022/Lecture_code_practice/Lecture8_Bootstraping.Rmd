...
ggplot(my_diamonds,aes(x=x/y))+  # there's a big hole in the middle # x and y are  rarely totally the same
...
boot(my_diamonds,mean_ratio,R=1000) # do sampling a lot of times and calculate the statistical variables
# do 1000 times
```
...
boot(my_diamonds,mean_ratio,R=10)
```
```{r}
...
```{r}
nlsfit<-nls(mpg~k/wt+b,mtcars,start=c(k=1,b=0)) # non linear fit
summary(nlsfit)

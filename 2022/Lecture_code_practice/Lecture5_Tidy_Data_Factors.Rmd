...
```
```{r}
table4b  # still make "year" a variable, and make the table longer
tidy4b<-table4b %>%
...
tidy2<-table2%>%
  pivot_wider(names_from = type,values_from = count) # anmes_from provide the names of the variables
tidy2
...
# now the separatio is correct
```
```{r}
#combine two columns into a cell
table5
table5%>%
  unite(new,century,year,sep="") # combine the two columns
```
# factors (chapter 15)
```{r}
x1<-c("Dec","Apr","Jan","Mar")
x2<-c("Dec","Apr","Jam","Mar")  # there is a typo
sort(x1) # sorted by alphabet, not the order of months
```
```{r}
month_levels<-c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
y2<-parse_factor(x2,levels = month_levels)  # "Jam" is not in the list
y2
```
```{r}
gss_cat
...
```
```{r}
ggplot(relig_summary,aes(tvhours,fct_reorder(relig,tvhours)))+geom_point() # the "relig" is reordered by the value of "tvhour"
```
```{r}
relig_summary%>%
  mutate(relig=fct_reorder(relig,tvhours))%>% # also the relig will bw in the order according to tvhours
